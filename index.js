...
.setThumbnail()
.addField(`Pronax | Teşekkürler`, `**Selamlar, Nokkel (Pronax'ın yapımcısı) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Pronax | Prefix`, `**Pronax Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`Pronax | Nasıl Kullanılır?`, `**Pronax botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`Pronax | Linkler`, `**Sohbet Kanalına -davet Yazmanız Yeterlidir**`)
.setFooter(`Pronax | Gelişmiş Türkçe Bot | 2020`)
...
var nokkel = [
`✨| -yardım | -genel | -moderasyon | -otocevap | -sayaç | -kayıtsistemi | -guard | -eklenti`,
`Made Wıth: Nokkel#0657`
]
  
client.on("ready", () => {
setInterval(function() {

         var random = Math.floor(Math.random()*(nokkel.length-0+1)+0);
         client.user.setActivity(nokkel[random], {"type": "WATCHING"});
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@!775410167494737970>`) return msg.channel.send(`> **Pronax | Prefix**\n\n>  **Sanırım beni etiketlediniz.**\n >  Buyurun prefix(ön ek)im \`${prefix}\``);
});
...
.setDescription(`<a:hypequad:775416596457193493> <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Pronax Bot | Log Sistemi")
...
if (!modlogkanal) return;
  ...
  .setColor("#ffd100")
    ...
    .setDescription(`<a:hypequad:775416596457193493> **Üye Sunucudan Yasaklandı!** \n<@!${message.user.id}>, ${message.user.tag}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setDescription(`<a:hypequad:775416596457193493> ${channel.name} **Adlı Metin Kanalı Oluşturuldu!**`)
                .setFooter(`Pronax Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
...
.setTitle("SES KANALI OLUŞTURULDU")
                .setDescription(`<a:hypequad:775416596457193493> ${channel.name} **Adlı Ses Kanalı Oluşturuldu!**`)
                .setFooter(`Pronax Bot | Log Sistemi Kanal ID: ${channel.id}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.MessageEmbed()
                    .setColor('#ffd100')
                .setDescription(`<a:hypequad:775416596457193493> ${channel.name} **Adlın Metin Kanalı  Silindi**`)
                .setFooter(`Pronax Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
            if (channel.type === "voice") {
...
.setTitle("SES KANALI SİLİNDİ")
                .setDescription(`<a:hypequad:775416596457193493> ${channel.name} **Adlı Ses Kanalı Silindi**`)
            .setFooter(`Pronax Bot | Log Sistemi  Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            }
    })
    ...
    if (küfür.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<a:hypequad:775416596457193493> **Hey ${message.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<a:hypequad:775416596457193493> **Hey ${newMessage.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<a:hypequad:775416596457193493> **Hey ${message.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<a:hypequad:775416596457193493> **Hey ${newMessage.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea' || msg.content.toLowerCase() == 's.a.' || msg.content.toLowerCase() == 'selam' || msg.content.toLowerCase() == 'slm') {
          try {
                  ...
                  return msg.reply('**<a:B_Dans:775416597170356285>Aleyküm Selam, Hoşgeldin.** ')
          } catch(err) {
            console.log(err);
          }
      }
...
client.on("guildMemberAdd", async member => {
  ...
  ///....
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:hypequad:775416596457193493> `"+ member.user.username + "`**Adlı Kullanıcı Aramıza Katıldı!** `" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
  }

  if (member.guild.memberCount == sayaç) {
    return client.channels.get(kanal).send(`<a:hypequad:775416596457193493> **Sayaç Sıfırlandı!** \`${member.guild.memberCount}\` **Kişiyiz!**`)
...
client.on("guildMemberRemove", async member => {
  ...
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:hypequad:775416596457193493> `" + member.user.username + "` **Adlı Kullanıcı Aramızdan Ayrıldı.**`" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
      }
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("<a:hypequad:775416596457193493> **Selam!** `" + member.user.username + "`**!kayıtol yazarak kayıt olabilirsin!**");
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("<a:hypequad:775416596457193493> `" + member.user.username + "`** Hoş Geldin! Otomatik Rolün Verildi Seninle Beraber** `" + member.guild.memberCount + "` **Kişiyiz!**");
